"""
File:    /gui/bug_report_dialog.py
Rol:     BugDialog (QDialog) — laat de gebruiker een bug of feature-
         aanvraag melden; GitHubClient commit een bestand naar de repo en
         opent een Issue (bug) of Pull Request (feature) via de GitHub
         REST API.
Applicatie: Project Generator
Versie:  1.0.2
Auteur:  Barremans
Changes: 1.0.2 - GitHub Personal Access Token ingevuld (scope 'repo',
                  barremans/Project_generator) — bugreport-functionaliteit
                  is nu effectief bruikbaar i.p.v. de expliciete
                  "geen token"-foutmelding uit v1.0.1.
Changes: 1.0.1 - Repo bevestigd: barremans/Project_generator
                  (https://github.com/barremans/Project_generator).
Changes: 1.0.0 - Geport vanuit ArticleSearch (bug_report_dialog.py v1.4.0,
                  auteur Bart Bossuyt) naar Project Generator:
                  (1) PySide6 -> PyQt6.
                  (2) GitHubClient.owner/repo aangepast naar
                      "barremans"/"Project_generator".
                  (3) "Je naam" wordt niet langer voorgevuld vanuit Azure
                      AD (permissions_azure — Project Generator heeft geen
                      AD-login), maar vanuit settings.default_author
                      (utils/settings.py AppSettings, dezelfde bron als de
                      hoofd-Instellingen). Zelfde fallback-gedrag als het
                      origineel: indien leeg, blijft het veld leeg én
                      bewerkbaar; indien gevuld, wordt het read-only
                      getoond (de gebruiker past dit dan aan via
                      Instellingen, niet in dit dialoogvenster).
                  (4) TOKEN is een LEGE placeholder — het origineel bevatte
                      een hardcoded ArticleSearch-PAT; dat token is
                      specifiek aan die repo gekoppeld en kan hier niet
                      hergebruikt worden. Genereer een nieuwe PAT (scope
                      'repo') voor barremans/Project_generator via GitHub
                      Settings -> Developer settings -> Personal access
                      tokens, en vul die hieronder in. Zonder geldig token
                      faalt submit_report() met een duidelijke fout (geen
                      stille silent-failure).
                  Overige logica (commit + branch + Issue/PR-aanmaak,
                  preview vóór verzenden) ONGEWIJZIGD overgenomen uit
                  ArticleSearch v1.4.0.
"""
import requests
import base64
import uuid
...
class GitHubClient:
    def __init__(self):
        # GitHub PAT (scope 'repo'), barremans/Project_generator.
        # Bewust hardcoded i.p.v. env var — zelfde afweging als in
        # core/updater.py en bij ArticleSearch: de app wordt via een
        # installer verspreid, per-machine configuratie is niet haalbaar.
        # Verloopt een token (401 Unauthorized bij commit/Issue/PR-aanmaak),
        # genereer dan een nieuwe PAT en vervang de waarde hieronder — nooit
        # hetzelfde verlopen token hergebruiken.
        self.token = "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"
        self.owner = "barremans"
        self.repo = "Project_generator"
    ...
    def submit_report(self, reporter: str, description: str, report_type: str) -> str:
        if not self.token:
            raise RuntimeError(
                "Geen GitHub-token geconfigureerd in GitHubClient "
                "(gui/bug_report_dialog.py). Vul self.token in met een "
                "geldige Personal Access Token (scope 'repo')."
            )
...
class BugDialog(QDialog):
    def __init__(self, parent=None, initial_type: Optional[str] = None,
                 initial_description: str = "", settings: Optional[AppSettings] = None):
        ...
        # Voorvullen vanuit settings.default_author (i.p.v. Azure AD, dat
        # Project Generator niet heeft) + read-only maken zodra gevuld.
        # Fallback: leeg/bewerkbaar zoals voorheen als er nog geen
        # standaard auteur is ingesteld.
        default_author = self.settings.default_author
        if default_author:
            self.name_input.setText(default_author)
    ...
    def submit_report(self):
        ...
        reply = QMessageBox.question(
            self, "Voorvertoning", preview_text,
